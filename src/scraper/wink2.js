...
const UA =
  "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTMhL, like Gecko) Chrome/147.0.0.0 Mobile Safari/537.36";
...
async function getImageInfo(fileKey) {
  const { client: api } = await getApi();
  ...
  const res = await api.post(
    "/api/file/image_cover_and_display_info_ext.json",
    body.toString(),
    {
      headers: {
        ...traceHeaders(),
  ...
  if (res.status >= 400 || res.data?.code !== 0) {
    throw new Error(`image info gagal: ${JSON.stringify(res.data)}`);
  }
...
async function startTranscode(fileKey) {
  ...
  const res = await api.post(
    "/api/file/image_trans_start.json",
    body.toString(),
    {
      headers: {
        ...traceHeaders(),
...
async function queryTranscode(id) {
  ...
  const res = await api.get(
    `/api/file/image_trans_query.json?${params.toString()}`,
    {
      headers: traceHeaders(),
    },
  );
...
async function waitTranscode(
  ...
  for (let i = 1; i <= maxTry; i++) {
    ...
    const image = data?.image || data?.url || data?.source_url || "";
    const imageTranscoded =
      data?.image_transcoded ||
      data?.transcoded_image ||
      data?.transcoded_url ||
      data?.image_url ||
      "";
    ...
    if (imageTranscoded) {
      return {
        source_url: image || fallbackSourceUrl,
        image_transcoded: imageTranscoded,
        raw: data,
      };
    }
  ...
  return {
    ...
    image_transcoded: fallbackSourceUrl,
    raw: last,
  };
}

async function delivery(sourceUrl, imageTranscoded, taskName) {
  const { client: api } = await getApi();
  const body = await baseParams({
    type: TASK_TYPE,
...
function extractResultUrl(data) {
  ...
  return (
    ...
    item?.result?.url ||
    item?.client_ext_params?.image_transcoded ||
    ""
  );
}
...
async function winkEnhance(image, { filename } = {}) {
  if (!image) throw new Error("image is required");

  const safeName = filename || `wink-${crypto.randomUUID()}.png`;
  const filePath = Buffer.isBuffer(image)
    ? path.join(os.tmpdir(), safeName)
    : image;
  const shouldCleanup = Buffer.isBuffer(image);
  ...
  try {
    ...
    await getImageInfo(uploaded.file_key);
